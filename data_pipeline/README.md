# Qwen3-VL Showee Data Pipeline

This module contains the lightweight annotation pipeline used to build Qwen3-VL video QA data from ShoweeHandv2.
...
Generated files are written under `data/`, `eval/`, and `runs/` relative to this module root unless `SHOWEE_PIPELINE_ROOT` is set. Those directories are ignored by Git.

## Path Configuration

The scripts default to paths inside `data_pipeline/`. On a shared server, set these variables to use an existing dataset/model:

```bash
export SHOWEE_PIPELINE_ROOT=<repo_root>/data_pipeline
...
The commands below assume the current working directory is `data_pipeline/`.
